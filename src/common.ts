...
export async function initialize(context: vscode.ExtensionContext): Promise<WebviewProvider> {
	try {
		...
		HostProvider.window.showMessage({
			...
			message: "Failed to initialize IoT AI Debugger's application state. Please restart the extension.",
		})
	}
...
async function showVersionUpdateAnnouncement(context: vscode.ExtensionContext) {
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`IoT AI Debugger version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			...
			if (lastShownAnnouncementId !== latestAnnouncementId) {
				...
				const message = previousVersion
					? `IoT AI Debugger has been updated to v${currentVersion}`
					: `Welcome to IoT AI Debugger v${currentVersion}`
				HostProvider.window.showMessage({
