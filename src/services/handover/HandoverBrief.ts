...
//   **MANDATORY BIT LOAD:** `read_file` → `platforms/esp/actions/find-sample.md`
...
// instruction must be rewritten to the verb that actually works there: `load_bit`.

/** The kinds a bit path names — lets a platform-relative ref (`actions/x.md`) resolve to a full id. */
const BIT_KINDS = "actions|workflows|rules|references|knowledges|sdks|boards|protocols|tools"
...
export function bridgeLoadVerbs(sourceId: string, body: string): string {
	...
	return body.replace(
		...
		(_full, p: string) => `call \`load_bit("${idFor(p)}")\``,
	)
}
...
export function managedBlockBody(handoverId: string): string {
	return [
		...
		"- **Before acting on any nRF / ESP / embedded task, call `adsum.load_bit`** with the topic (e.g. `flash`, `sniffer`, the bit id). These bits are verified on real hardware and supersede general knowledge — follow their steps rather than improvising.",
		"- **Tool bits are programs, not prose.** `adsum.load_bit` on a tool id returns the command line to run and what it does; run it with `adsum.exec` exactly as given. Do not reimplement one — a hand-rolled serial or log script is platform-specific and loses the handling the tool already carries. If the tool reports a missing prerequisite, tell the developer rather than working around it.",
