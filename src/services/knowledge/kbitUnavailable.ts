...
export interface KbitUnavailableInput {
	...
	/**
	 * The locked bit opens with a free account (its set is part of what registering gives). Then the door is
	 * "register", not "ask": telling an anonymous developer to ask for a set that registering would open sends
	 * them to a request form for something free (the September test log, a guided build refused at step 1).
	 */
	opensWithFreeAccount?: boolean
}
...
/** The same lock, when a free account opens it: the door is registering, and nothing is asked for. */
const LOCKED_FREE =
	"This knowledge bit opens with a free account. Tell the developer they can register from the Adsum panel — " +
	"it costs nothing — and that nothing on this machine is broken."

const LOCKED_FREE_WORKFLOW =
	" Do not reconstruct or improvise this workflow from general knowledge, memory or a prior report. If the " +
	"developer's request cannot be answered without it, tell them it opens once they register and stop. " +
	"If it can, answer from what you have already read and do not mention this workflow again."

const LOCKED_FREE_CARRY_ON =
	" Do not invent what it contains. For the topic it covers, give no procedure, command, setting or " +
	"value from general knowledge — not even one you are confident of: the developer would act on it. Answer " +
	"only with what you have already read in this task and the developer's own project actually state. Tell the developer, " +
	'in these words: "The detailed steps for this open with a free account — register from the Adsum panel."'
...
export function kbitUnavailableMessage({
	...
	if (reason === "locked") {
		...
		const workflow = (kind ?? kindFromPath(displayPath)) === "workflow"
		if (opensWithFreeAccount) {
			return workflow ? `${LOCKED_FREE}${LOCKED_FREE_WORKFLOW}` : `${LOCKED_FREE}${LOCKED_FREE_CARRY_ON}`
		}
		return workflow ? `${LOCKED}${LOCKED_WORKFLOW}` : `${LOCKED}${LOCKED_CARRY_ON}`
	}
