...
describe("a locked bit a free account opens says register, not ask", () => {
	const base = { reason: "locked" as const, antiImprovise: " Do not improvise." }

	it("a knowledge bit a free account opens: register, costs nothing, never 'on request', never a group name", () => {
		const m = kbitUnavailableMessage({
			...base,
			displayPath: "products/fanstel/blg20x/beats/s1-hardware-truth.md",
			opensWithFreeAccount: true,
		})
		assert.match(m, /opens with a free account/)
		assert.match(m, /register from the Adsum panel/)
		assert.doesNotMatch(m, /on request|ask for more details/)
		assert.doesNotMatch(m, /cellular-advanced|entitlement|group/)
		assert.match(m, /Do not invent what it contains/)
	})

	it("a workflow a free account opens stops only when needed, and names registering as the way in", () => {
		const m = kbitUnavailableMessage({ ...base, displayPath: "workflows/blg20x-first-run.md", opensWithFreeAccount: true })
		assert.match(m, /opens once they register and stop/)
		assert.doesNotMatch(m, /currently unavailable/)
	})

	it("without the flag the request sentence is unchanged, so a set that registering does not open still says ask", () => {
		const m = kbitUnavailableMessage({ ...base, displayPath: "products/fanstel/blg20x/adv/the-seam.md" })
		assert.match(m, /opened on request/)
		assert.doesNotMatch(m, /free account/)
	})
})
