...
function productLine(): string {
	...
	// [14 Sep 2026, H5] One row per product family. The BLG20x is the second entry; the follow-up is to build
	// these rows from the published product indexes (see PRODUCT_SIGNATURES in WorkspaceClassifier.ts).
	return (
		...
		"| BLG20x, BLG20, BM20 module, nRF9151 with nRF54LM20B on one board, a satellite or cellular gateway with two debug headers, or a *Continue the BLG20x first run* opener | " +
		"load `products/fanstel/blg20x/PRODUCT.md` **first** — it names the bit for each question about this board |\n\n"
	)
}
