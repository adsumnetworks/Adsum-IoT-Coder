...
import { type EditResultContext, formatEditResult } from "./edit-result"
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	taskResumption: (
		...
		const taskResumptionMessage = `[TASK RESUMPTION] ${
			...
			wasRecent && !hasPendingFileContextWarnings
				? "\n\nIMPORTANT: If the last tool use was a replace_in_file or write_to_file that was interrupted, the file was reverted back to its original state before the interrupted edit, and you do NOT need to re-read the file as you already have its up-to-date contents. If that interrupted write was LARGE, the response stream likely hit the per-response output limit — do NOT retry the same content verbatim: retry as a SHORTER write (summarize long tables; point to on-disk artifacts for the full data) or split the content into two writes."
				: ""
		}`
	...
	/**
	 * A message the developer sent while the turn was already running.
	 *
	 * Delivered at the next turn boundary alongside the tool results, which is the one place it can go
	 * without either interleaving with the model's own stream or reading as output FROM a tool. The
	 * wrapper has three jobs, and each of them is load-bearing:
	 *
	 *  - `<user_message>` is in USER_CONTENT_TAGS, so loadContext expands @-mentions and slash commands
	 *    inside it. The seam's old `[note ...]` label did not, and a path a driver typed arrived as text.
	 *  - It says the message came from a person, never from a tool or from the agent's own observation.
	 *    The agent must be able to tell "a human told me this" from "I measured this".
	 *  - It says this is the SAME turn continuing, not a new task. Without that the model tends to treat
	 *    a mid-turn message as a fresh instruction and abandons the work in flight.
	 */
	queuedUserMessage: (text: string, from?: string): string => {
		const who = from?.trim() ? `${from.trim()} sent` : "The user sent"
		return `${who} a new message while you were working:

<user_message>
${text}
</user_message>

This is how Adsum IoT Coder surfaces messages the user sends mid-turn — within the running turn, often alongside the next tool result, rather than as a separate conversation turn. Address the message above as you continue this turn.`
	},
...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	/**
	 * Successful edit where the user modified the proposed content in the diff view.
	 * The user's delta is always shown (that is the point of this path), but as a diff —
	 * the whole post-edit file is no longer echoed for an EXISTING file. See edit-result.ts.
	 */
	fileEditWithUserChanges: (
		...
		formatEditResult({
			...
			fileExisted: context?.fileExisted ?? true,
	...
	/** Successful edit, content saved exactly as proposed. See edit-result.ts for the format. */
