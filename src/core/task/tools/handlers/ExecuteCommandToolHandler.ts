	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if ((!requiresApprovalPerLLM && autoApproveSafe) || (requiresApprovalPerLLM && autoApproveSafe && autoApproveAll)) {
			...
			showNotificationForApproval(
				`Adsum IoT Coder wants to execute a command: ${actualCommand}`,
		...
		// CRA funnel: the golden-path SBOM is generated by a shell command (west ncs-sbom / esp-idf-sbom /
		// west spdx), not write_to_file — so detect it here. Keyed on the command (never sent); once/task.
		emitCraMilestoneForCommand(config, actualCommand)

		// The terminal boundary: fold long output BEFORE it enters context. The user has already seen the full
		// output streamed into the terminal and the UI (executeCommandTool did that) — this folds only the copy
		// handed back to the model. Measured leaks through this exact path: a `.bat` build at ~43K tokens, an
		// `idf.py build` at ~36K tokens, a failed Remove-Item at ~32K tokens of PowerShell spew. Output under
		// the thresholds is returned byte-identical.
		return foldCommandOutput(result, { source: "the terminal" })
	}
}
