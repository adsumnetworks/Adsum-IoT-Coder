...
describe("the entry surface shows one notice at a time", () => {
	...
	it("falls through the order as each becomes ineligible", () => {
		expect(oneNotice({ dock: true, registered: true, upgrade: true, review: true })).toBe("dock")
		expect(oneNotice({ registered: true, upgrade: true, review: true })).toBe("registered")
	...
	it("the order is the policy, and it is pinned here", () => {
		expect([...NOTICE_ORDER]).toEqual(["cra", "dock", "registered", "upgrade", "review"])
	})
})
