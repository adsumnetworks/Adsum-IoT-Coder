...
8. **Help you *start* the top fix — in dependency order.** Offer, via `ask_followup_question`, to *begin* closing the top gap (e.g. "Want me to start adding MCUboot secure boot?") — route into the existing add-feature workflow. Offer gaps in **dependency order**, not just severity. Framed as **help you start**: a change you apply is **"started — you must build, flash, and verify"**, NEVER "fixed" / "done" / "resolved" / "now compliant" (a successful *build* is NOT proof a security feature works). **On the bundled sample, NEVER modify it — show the diff only** (it is read-only and ours). The user owns the result.
   - **Integrity guard for any fix you apply:** (a) it must close a **real, evidenced** gap from the table — if, on opening the code, you find the gap was wrong (the setting was already there), **say so and stop**; do NOT invent a new rationale to justify editing anyway. (b) Change **only** what the gap needs — don't rewrite unrelated working logic or fold in a functional change and call it a security fix. (c) After applying, the report row and the chat both stay **"started — unverified"** (built ≠ flashed ≠ verified) — never restyle the row to "Fixed ✅".

## Honesty rules
- Real evidence per row or "unknown" — never invent a status.
- **Verify the negative AND the positive — read the merged `.config` both ways.** A ❌/gap requires the named Kconfig symbol genuinely absent in the **merged `.config`** (or `# CONFIG_x is not set`); a symbol that is **present** (e.g. `CONFIG_BT_SMP=y`) is NOT a gap. The mirror is just as binding: a **✅ requires the check's named symbol present** — if it shows `not set`, it is not a ✅ regardless of adjacent symbols, and you must not narrate a pass the config contradicts (don't call secure boot ✅ when `# CONFIG_BOOTLOADER_MCUBOOT is not set`, and don't invent a "child image" the build didn't produce). Multi-image features (MCUboot/B0): confirm the bootloader sub-image was actually built, not just requested in `prj.conf`. (One run fabricated "SMP off" on a build where SMP+LE-SC were on; another stamped secure boot ✅ on a build where MCUboot was not set and no bootloader image was built. Both ignored the evidence already on screen.)
...
- **Never write "fixed" / "done" / "resolved" / "✅ FIXED" for a gap** — in the report header, posture table, chat, **or the final completion/summary message** (the model has slipped "Top gap fixed" into the closing summary even when the table itself was clean). A change you applied is **"started — unverified"** until the user builds, flashes, and confirms; a clean build is not verification. Phrase it as **"started (Kconfig added to `prj.conf`) — you must build, flash, and verify"**; don't restyle a ❌/⚠️ row to "✅ fixed".
