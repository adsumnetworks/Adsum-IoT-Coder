...
- **Verify the negative before you call something missing.** A ❌/gap requires the Kconfig symbol to be genuinely absent in the **merged `.config`** (or shown as `# CONFIG_x is not set`). If the symbol is **present** (e.g. `CONFIG_BT_SMP=y`), it is NOT a gap — do not report it as one, and do not headline a "critical" finding that contradicts evidence you already read. (A real run invented "SMP not enabled / BLE is plaintext" on a build where SMP + LE Secure Connections were on.)
