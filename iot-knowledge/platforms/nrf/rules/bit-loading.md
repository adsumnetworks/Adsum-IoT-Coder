...
# nRF Platform Rule: Bit Loading (rules/bit-loading.md)

> **MANDATORY FIRST LOAD:** if the universal `rules/bit-loading.md` (`iot-knowledge/rules/bit-loading.md`)
...
`rules/bit-loading.md`. This table is the nRF-specific instance of it.)*
...
4. **You're inside `debug-loop` and it tells you to load `actions/build.md` via a `MANDATORY BIT LOAD` directive** — the Workflow is invoking an Action; obey the directive.
